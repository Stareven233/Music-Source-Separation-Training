...
def parse_args_train(dict_args: Union[argparse.Namespace, Dict, None]) -> argparse.Namespace:
  ...
  parser.add_argument("--metric_for_scheduler",
                      default="sdr",
                      choices=['k_sdr', 'sdr', 'l1_freq', 'si_sdr', 'log_wmse', 'aura_stft', 'aura_mrstft', 'bleedless', 'fullness', 'l1_snr'],
  ...
  parser.add_argument("--prefetch_factor", type=int, default=None, help="dataloader prefetch_factor")
  ...
  parser.add_argument("--freeze_layers", nargs="+", type=str, help="List of layers to freeze. Use prefixes e.g. layer1 - will freeze all layers whose names "
  ...
  get_internal_loss = (args.model_type in ('mel_band_conformer',) or 'roformer' in args.model_type) and not args.use_standard_loss
...
def parse_args_valid(dict_args: Union[Dict, None]) -> argparse.Namespace:
  """
    ...
    """
  parser = argparse.ArgumentParser()
  parser.add_argument("--model_type", type=str, default='mdx23c', help="One of mdx23c, htdemucs, segm_models, mel_band_roformer,"
  ...
  parser.add_argument("--draw_spectro",
                      type=float,
                      default=0,
  ...
  parser.add_argument("--metrics",
                      ...
                      type=str,
                      default=["sdr"],
                      choices=['k_sdr', 'sdr', 'l1_freq', 'si_sdr', 'neg_log_wmse', 'aura_stft', 'aura_mrstft', 'bleedless', 'fullness', 'l1_snr'],
                      help='List of metrics to use.')
  parser.add_argument("--lora_checkpoint_peft", type=str, default='', help="Initial checkpoint to LoRA weights")
  parser.add_argument("--lora_checkpoint_loralib", type=str, default='', help="Initial checkpoint to LoRA weights")
...
def parse_args_inference(dict_args: Union[Dict, None]) -> argparse.Namespace:
  """
    ...
    """
  parser = argparse.ArgumentParser()
  parser.add_argument("--model_type", type=str, default='mdx23c', help="One of bandit, bandit_v2, bs_roformer, htdemucs, mdx23c, mel_band_roformer,"
...
def get_model_from_config(model_type: str, config_path: str) -> Tuple[nn.Module, Union[ConfigDict, OmegaConf]]:
  ...
  if model_type == 'mdx23c':
    ...
    model = ConformerMSS(core=NeuralModel(**config.model), n_fft=config.stft.n_fft, hop_length=config.stft.hop_length, win_length=getattr(config.stft, 'win_length', config.stft.n_fft), center=config.stft.center)
  elif model_type == 'mel_band_conformer':
...
def wandb_init(args: argparse.Namespace, config: Union[ConfigDict, OmegaConf], batch_size: int) -> None:
  ...
  if args.wandb_offline:
    wandb.init(mode='offline', project='msst', name=gen_wandb_name(args, config), config={'config': config, 'args': args, 'device_ids': args.device_ids, 'batch_size': batch_size})
  elif args.wandb_key is None or args.wandb_key.strip() == '':
    wandb.init(mode='disabled')
  else:
    wandb.login(key=args.wandb_key)
    wandb.init(project='msst', name=gen_wandb_name(args, config), config={'config': config, 'args': args, 'device_ids': args.device_ids, 'batch_size': batch_size})
